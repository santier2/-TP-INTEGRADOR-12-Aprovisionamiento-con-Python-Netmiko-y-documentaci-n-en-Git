...
cfg_sw1 = [
    ...
    " switchport trunk allowed vlan 239,230,231,232,1299",
    " no shutdown",
    "exit",
]
...
cfg_sw2 = [
    ...
    "interface Ethernet0/1",
    " switchport mode access",
    " switchport access vlan 1299",   # PC remota ahora en VLAN Gestión
    " no shutdown",
    "exit",
    "interface Ethernet0/0",
    " switchport trunk encapsulation dot1q",
...
cfg_r1 = [
    ...
    "/interface vlan add name=VLAN230 vlan-id=230 interface=ether2",
    "/interface vlan add name=VLAN231 vlan-id=231 interface=ether2",
    "/interface vlan add name=VLAN232 vlan-id=232 interface=ether2",

    # Direccionamiento para las VLANs de usuario (VLSM aplicado)
    "/ip address add address=10.10.12.33/27 interface=VLAN230",  # Ventas
    "/ip address add address=10.10.12.65/28 interface=VLAN231",  # Técnica
    "/ip address add address=10.10.12.81/29 interface=VLAN232",  # Visitantes
    ...
    "/ip pool add name=POOL_VLAN230 ranges=10.10.12.34-10.10.12.62",
    "/ip dhcp-server add name=DHCP230 interface=VLAN230 lease-time=1h address-pool=POOL_VLAN230",
    "/ip dhcp-server network add address=10.10.12.32/27 gateway=10.10.12.33 dns-server=8.8.8.8",

    "/ip pool add name=POOL_VLAN231 ranges=10.10.12.66-10.10.12.78",
    "/ip dhcp-server add name=DHCP231 interface=VLAN231 lease-time=1h address-pool=POOL_VLAN231",
...
cfg_r2 = [
    ...
    "/interface bridge vlan add bridge=br-remote vlan-ids=230 tagged=ether1,ether2",
    "/interface bridge vlan add bridge=br-remote vlan-ids=231 tagged=ether1,ether2",
    "/interface bridge vlan add bridge=br-remote vlan-ids=232 tagged=ether1,ether2",
]
